...
namespace XWIN.Handlers
{
    ...
    public class NotifyHandler : Handler
    {
        ...
        private void AddMenuStrip()
        {
            ...
            AddMenuItem("Открыть X-WIN", OnOpenClick);
            AddMenuItem("Режим", delegate { }, modeItems.Values.ToArray());
            AddMenuItem("Проверить обновления", OnUpdateClick);
            AddMenuItem("О проекте", OnAboutClick);
            AddMenuItem("Закрыть", OnCloseClick);
